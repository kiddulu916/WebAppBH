...
- `base_tool.py` — Abstract base class (e.g. `ReconTool`, `ApiTestTool`) with subprocess runner, cooldown check, scope-check, and DB insert helpers
- `tools/` — One file per external tool, subclassing the base. Implements `build_command()`/`parse_output()` (recon) or `execute()` (api/fuzzing)
- `pipeline.py` — Ordered `Stage` list, runs tools concurrently within each stage via `asyncio.gather`, checkpoints progress in `job_state` table
- `concurrency.py` — Semaphore-based concurrency control with `WeightClass` enum (HEAVY/LIGHT)
- `main.py` — Entry point calling `listen_queue` and running pipeline per message, with heartbeat loop
- `requirements.txt` — Worker-specific Python deps

Check that new workers or tool additions follow this exact structure.
...
Valid models: `Target`, `Asset`, `Identity`, `Location`, `Observation`, `CloudAsset`, `Parameter`, `Vulnerability`, `JobState`, `Alert`, `ApiSchema`
...
Redis stream names are fixed: `recon_queue`, `fuzzing_queue`, `cloud_queue`, `api_queue`, `events:{target_id}`

Messages must use `push_task()` from `lib_webbh.messaging`. Flag any direct Redis calls for queue operations.
...
- Dockerfiles follow multi-stage build pattern (see `docker/Dockerfile.recon` as reference)
