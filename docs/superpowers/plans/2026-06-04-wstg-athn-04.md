...
```python
    ...
    async def _probe_forced_browsing(
        ...
        async with httpx.AsyncClient(verify=False, follow_redirects=False, timeout=10) as client:
            for path in paths:
                url = urljoin(base_url, path)
                if scope_manager.is_in_scope(url) != ScopeResult.IN_SCOPE:
                    continue
            ...
            if scope_manager.is_in_scope(admin_url) == ScopeResult.IN_SCOPE:
    ...
    async def _probe_sqli_bypass(
        ...
        async with httpx.AsyncClient(verify=False, follow_redirects=False, timeout=10) as client:
            for login_url in login_urls:
                if scope_manager.is_in_scope(login_url) != ScopeResult.IN_SCOPE:
                    continue
