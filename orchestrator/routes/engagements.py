...
async def search_engagement(body: EngagementSearchRequest):
    ...
    try:
        ...
        logger.warning("Engagement search failed", extra={"error": redact_sensitive(str(exc))})
    ...
    if len(candidates) == 1:
        ...
        try:
            ...
            logger.warning("Auto-fetch failed", extra={"error": redact_sensitive(str(exc))})
...
async def fetch_engagement_endpoint(body: EngagementFetchRequest):
    ...
    try:
        ...
        logger.warning("Engagement fetch failed", extra={"error": redact_sensitive(str(exc))})
        raise HTTPException(status_code=502, detail=f"Program fetch failed: {exc}") from exc
