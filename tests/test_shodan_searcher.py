"""Tests for ShodanSearcher — optional Shodan API integration."""
...
class TestShodanSearcher:
    ...
    async def test_skips_when_no_api_key(self):
        """Should return None when SHODAN_API_KEY not set."""
    ...
    async def test_returns_stats_when_key_set(self):
        ...
        with patch.dict("os.environ", {"SHODAN_API_KEY": "test-key"}):
            ...
            with patch("aiohttp.ClientSession") as mock_cls:
                ...
                mock_resp.json = AsyncMock(return_value={"example.com": "1.2.3.4"})
                mock_ctx = AsyncMock()
                ...
                with patch.object(tool, "save_asset", new_callable=AsyncMock, return_value=1), \
                     ...
                     patch.object(tool, "scope_check", new_callable=AsyncMock, return_value=True), \
                     patch("workers.info_gathering.tools.shodan_searcher.asyncio.sleep", new_callable=AsyncMock):
                    result = await tool.execute(
                        target_id=1, domain="example.com", scope_manager=AsyncMock()
                    )
