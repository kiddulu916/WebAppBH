...
def get_container_name() -> str:
    return os.environ.get("HOSTNAME", "authentication-unknown")


async def run_pipeline(target_id: int) -> None:
    """Run the authentication pipeline for a target, with credential check."""
    creds_path = Path(f"shared/config/{target_id}/credentials.json")
    if not creds_path.exists():
        logger.info(f"No credentials found for target {target_id}, skipping authentication testing")
...
async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single auth_queue message."""
    ...
    log.info("Received authentication testing request", extra={"msg_id": msg_id})
