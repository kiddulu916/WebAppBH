...
logger = setup_logger("auth-pipeline")


@dataclass
class Stage:
    name: str
    tool_classes: list[type[AuthenticationTool]]
...
STAGES = [
    ...
    Stage("remember_password", [RememberPasswordTester]),
    Stage("browser_cache", [BrowserCacheWeaknessTester]),
    Stage("weak_password_policy", [PasswordPolicyTester]),
    Stage("security_questions", [SecurityQuestionTester]),
    Stage("password_change", [PasswordChangeTester]),
    Stage("multi_channel_auth", [MultiChannelAuthTester]),
]
...
class Pipeline(CheckpointMixin):
    """Orchestrates the 10-stage authentication pipeline with checkpointing."""
    ...
    def _filter_stages(self, playbook: dict | None) -> list[Stage]:
        ...
        worker_stages = get_worker_stages(playbook, "authentication")
