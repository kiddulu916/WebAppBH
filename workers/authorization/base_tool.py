"""Abstract base class for authorization tool wrappers."""
...
from workers.authorization.concurrency import WeightClass, get_semaphore

logger = setup_logger("authz-tool")

TOOL_TIMEOUT = int(os.environ.get("TOOL_TIMEOUT", "600"))
COOLDOWN_HOURS = int(os.environ.get("COOLDOWN_HOURS", "24"))


class AuthorizationTool(InfrastructureMixin, ABC):
    """Base class for all authorization tool wrappers.
    ...
    def parse_output(self, stdout: str) -> list:
        ...
        Returns:
            list[dict] with observation details for authorization findings
        """
    ...
    async def execute(
        ...
        """
        log = logger.bind(target_id=target_id, asset_type="authz")
