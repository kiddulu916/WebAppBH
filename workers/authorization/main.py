"""Authorization worker entry point.

Listens on ``authorization_queue`` (priority-tiered) and runs the 4-stage
authorization pipeline for each incoming target.
"""
...
def get_container_name() -> str:
    return os.environ.get("HOSTNAME", "authorization-unknown")


async def run_pipeline(target_id: int) -> None:
    """Run the authorization pipeline for a target, with credential check."""
    creds_path = Path(f"shared/config/{target_id}/credentials.json")
    if not creds_path.exists():
        logger.info(f"No credentials found for target {target_id}, skipping authorization testing")
...
async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single authz_queue message."""
    ...
    log.info("Received authorization testing request", extra={"msg_id": msg_id})
...
async def main() -> None:
    """Entry point: listen on authorization_queue forever."""
    consumer_group = "authorization_group"
    consumer_name = get_container_name()
    logger.info("Authorization worker starting", extra={"container": consumer_name})

    async for message in listen_priority_queues(
        "authorization_queue", consumer_group, consumer_name
    ):
        try:
