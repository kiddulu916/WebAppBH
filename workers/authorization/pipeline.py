"""Authorization pipeline: 4 sequential stages."""
...
from workers.authorization.tools import (
...
logger = setup_logger("authz-pipeline")


@dataclass
class Stage:
    name: str
    tool_classes: list[type[AuthorizationTool]]


STAGES = [
    Stage("directory_traversal", [DirectoryTraversalTester]),
    Stage("authz_bypass", [AuthzBypassTester]),
    Stage("privilege_escalation", [PrivilegeEscalationTester]),
    Stage("idor", [IdorTester]),
]
...
class Pipeline(CheckpointMixin):
    """Orchestrates the 4-stage authorization pipeline with checkpointing."""
    ...
    def _filter_stages(self, playbook: dict | None) -> list[Stage]:
        ...
        worker_stages = get_worker_stages(playbook, "authorization")
