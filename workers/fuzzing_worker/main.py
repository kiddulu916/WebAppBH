"""Fuzzing worker entry point.

Listens on ``fuzzing_queue`` and runs the fuzzing
...
logger = setup_logger("fuzzing-worker")

HEARTBEAT_INTERVAL = int(os.environ.get("HEARTBEAT_INTERVAL", "30"))


def get_container_name() -> str:
    return os.environ.get("HOSTNAME", "fuzzing-worker-unknown")


async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single fuzzing_queue message."""
    ...
    log.info("Received fuzzing task", extra={"msg_id": msg_id})
...
async def main() -> None:
    """Entry point: listen on fuzzing_queue forever."""
    container_name = get_container_name()
    logger.info("Fuzzing worker starting", extra={"container": container_name})
