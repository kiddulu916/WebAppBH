"""Abstract base class for identity management tool wrappers."""
...
from workers.identity_mgmt.concurrency import WeightClass, get_semaphore

logger = setup_logger("identity-tool")

TOOL_TIMEOUT = int(os.environ.get("TOOL_TIMEOUT", "600"))
COOLDOWN_HOURS = int(os.environ.get("COOLDOWN_HOURS", "24"))


class IdentityMgmtTool(InfrastructureMixin, ABC):
    """Base class for all identity management tool wrappers.
    ...
    async def execute(
        ...
        """
        log = logger.bind(target_id=target_id, asset_type="identity")
