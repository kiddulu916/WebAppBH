"""Identity Management worker entry point.

Listens on ``identity_mgmt_queue`` (priority-tiered) and runs the 5-stage
identity management pipeline for each incoming target.
"""
...
logger = setup_logger("identity-mgmt")

HEARTBEAT_INTERVAL = int(os.environ.get("HEARTBEAT_INTERVAL", "30"))


def get_container_name() -> str:
    return os.environ.get("HOSTNAME", "identity-mgmt-unknown")


async def run_pipeline(target_id: int) -> None:
    """Run the identity management pipeline for a target, with credential check."""
    creds_path = Path(f"shared/config/{target_id}/credentials.json")
    if not creds_path.exists():
        logger.info(f"No credentials found for target {target_id}, skipping identity management")
...
async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single identity_queue message."""
    ...
    log.info("Received identity management request", extra={"msg_id": msg_id})
...
async def main() -> None:
    """Entry point: listen on identity_mgmt_queue forever."""
    consumer_group = "identity_mgmt_group"
    consumer_name = get_container_name()
    logger.info("Identity Management starting", extra={"container": consumer_name})

    async for message in listen_priority_queues(
        "identity_mgmt_queue", consumer_group, consumer_name
    ):
        try:
