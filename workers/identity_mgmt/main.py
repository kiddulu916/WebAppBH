...
logger = setup_logger("identity_mgmt")
...
async def main() -> None:
    ...
    logger.info("Identity management worker starting")
    ...
    async for message in listen_priority_queues(
        ...
        try:
            creds_path = SHARED_CONFIG / str(target_id) / "credentials.json"
            if not creds_path.exists():
                logger.info("No credentials, skipping", extra={"target_id": target_id})
            ...
            playbook_path = SHARED_CONFIG / str(target_id) / "playbook.json"
