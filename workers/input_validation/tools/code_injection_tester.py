...
class CodeInjectionTester(InputValidationTool):
    """Test for code injection vulnerabilities."""

    name = "code_injection_tester"
    ...
    async def execute(
        ...
        **kwargs,
        ...
        """Execute code injection tests against target URLs."""
        found = 0
        vulnerable = 0

        urls = await self._get_all_url_assets(target_id)
        payloads = [";phpinfo();", ";system('id');", ";eval('echo 123');"]
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(test_url, headers=headers, timeout=10) as resp:
                            ...
                            if "phpinfo" in response_text or "uid=" in response_text or "123" in response_text:
                                await self._save_vulnerability(
                                    target_id=target_id,
                                    asset_id=asset_id,
                                    severity="critical",
                                    title="Code Injection Vulnerability",
                                    description=f"Code injection found at {url} with payload: {payload}",
                                    poc=test_url,
                                )
                                vulnerable += 1
                                break
