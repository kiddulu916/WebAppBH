"""Reflected XSS testing tool."""

from __future__ import annotations

import aiohttp
from lib_webbh.scope import ScopeManager
from workers.input_validation.base_tool import InputValidationTool
from workers.input_validation.concurrency import WeightClass


class ReflectedXssTester(InputValidationTool):
    """Test for reflected XSS vulnerabilities."""

    name = "reflected_xss_tester"
    weight_class = WeightClass.LIGHT

    async def execute(
        self,
        target,
        scope_manager: ScopeManager,
        target_id: int,
        container_name: str,
        headers: dict | None = None,
        **kwargs,
    ) -> dict:
        """Execute reflected XSS tests against target URLs."""
        found = 0
        vulnerable = 0

        # Get URLs to test
        urls = await self._get_all_url_assets(target_id)
        payloads = self.get_injection_payloads("xss")

        for asset_id, url in urls:
            for payload in payloads:
                # Test by appending payload as query parameter
                test_url = f"{url}?test={payload}"
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(test_url, headers=headers, timeout=10) as resp:
                            response_text = await resp.text()
                            if self.detect_vulnerability(response_text, "xss"):
                                # Save vulnerability
                                await self._save_vulnerability(
                                    target_id=target_id,
                                    asset_id=asset_id,
                                    severity="high",
                                    title="Reflected XSS Vulnerability",
                                    description=f"Reflected XSS found at {url} with payload: {payload}",
                                    poc=test_url,
                                )
                                vulnerable += 1
                                break  # One vuln per URL is enough
                except Exception:
                    continue
            found += 1

        return {"found": found, "vulnerable": vulnerable}