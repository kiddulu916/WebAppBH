"""MSSQL-specific SQL injection testing using sqlmap."""
...
class SqlmapMssqlTool(InputValidationTool):
    """Test for MSSQL injection vulnerabilities using sqlmap."""

    name = "sqlmap_mssql"
    weight_class = WeightClass.HEAVY

    def build_command(self, target: str, headers: dict | None = None) -> list[str]:
        """Build the sqlmap command for MSSQL injection testing."""
        cmd = ["sqlmap", "-u", target, "--dbms=MSSQL", "--batch", "--random-agent"]
        if headers:
    ...
    def parse_output(self, stdout: str) -> list[dict]:
        """Parse sqlmap output for MSSQL injection results."""
        results = []
        return results
    ...
    async def execute(
        ...
        **kwargs,
        ...
        """Execute MSSQL injection tests against target URLs."""
        found = 0
        vulnerable = 0
