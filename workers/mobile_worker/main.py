"""Mobile testing worker entry point.

Listens on ``mobile_queue`` and runs the 5-stage
mobile testing pipeline for each incoming target.
"""
...
logger = setup_logger("mobile-worker")

HEARTBEAT_INTERVAL = int(os.environ.get("HEARTBEAT_INTERVAL", "30"))


def get_container_name() -> str:
    return f"mobile-worker-{os.environ.get('HOSTNAME', 'unknown')}"


async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single mobile_queue message."""
    ...
    log.info("Received mobile testing task", extra={"msg_id": msg_id})
...
async def main() -> None:
    """Entry point: listen on mobile_queue forever."""
    container_name = get_container_name()
    logger.info("Mobile testing worker starting", extra={"container": container_name})
