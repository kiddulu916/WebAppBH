"""Abstract base class for session management tool wrappers."""
...
logger = setup_logger("session-tool")

TOOL_TIMEOUT = int(os.environ.get("TOOL_TIMEOUT", "600"))
COOLDOWN_HOURS = int(os.environ.get("COOLDOWN_HOURS", "24"))


class SessionMgmtTool(InfrastructureMixin, ABC):
    """Base class for all session management tool wrappers.
    ...
    def parse_output(self, stdout: str) -> list:
        ...
        Returns:
            list[dict] with observation details for session findings
        """
    ...
    async def execute(
        ...
        """
        log = logger.bind(target_id=target_id, asset_type="session")
