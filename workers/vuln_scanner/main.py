"""Vuln scanner worker entry point.

Listens on ``vulnscan_queue`` and runs the 3-stage
vulnerability scanning pipeline for each incoming target.
"""
...
from workers.vuln_scanner.pipeline import Pipeline

logger = setup_logger("vulnscan-worker")

HEARTBEAT_INTERVAL = int(os.environ.get("HEARTBEAT_INTERVAL", "30"))


def get_container_name() -> str:
    return os.environ.get("HOSTNAME", "vulnscan-worker-unknown")


async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single vulnscan_queue message."""
    ...
    log.info("Received vulnscan task", extra={"msg_id": msg_id})
...
async def main() -> None:
    """Entry point: listen on vulnscan_queue forever."""
    container_name = get_container_name()
    logger.info("Vuln scanner worker starting", extra={"container": container_name})

    await listen_queue(
        queue="vulnscan_queue",
        group="vulnscan_group",
