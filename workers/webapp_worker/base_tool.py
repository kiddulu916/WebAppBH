"""Abstract base class for web-application analysis tool wrappers."""
...
logger = setup_logger("webapp-tool")
...
class ToolType(Enum):
    """Execution model of a webapp analysis tool."""
...
class WebAppTool(ABC):
    """Base class for all web-application analysis tool wrappers.

    Subclasses must set ``name``, ``tool_type``, and ``weight_class`` class
    ...
    async def _save_observation(
        ...
        """Insert an Observation row and return its id."""
        async with get_session() as session:
            obs = Observation(
                asset_id=asset_id,
