"""Webapp worker entry point.

Listens on ``webapp_queue`` and runs the web application testing
...
logger = setup_logger("webapp-worker")

HEARTBEAT_INTERVAL = int(os.environ.get("HEARTBEAT_INTERVAL", "30"))


def get_container_name() -> str:
    return os.environ.get("HOSTNAME", "webapp-worker-unknown")


async def handle_message(msg_id: str, data: dict) -> None:
    """Process a single webapp_queue message."""
    target_id = data.get("target_id")
    action = data.get("action", "webapp_testing")
...
async def main() -> None:
    """Entry point: listen on webapp_queue forever."""
    container_name = get_container_name()
    logger.info("Webapp worker starting", extra={"container": container_name})
