"""JsMiner — Stage 2 static JS analysis for endpoint extraction.

Reads saved JS files from disk and runs the ``jsminer`` CLI tool
...
logger = setup_logger("jsminer")

# Base directory for raw JS file storage.  Tests patch this to tmp_path.
JS_DIR = "/app/shared/raw"


class JsMiner(WebAppTool):
    """Extract endpoints from saved JS files using JsMiner CLI."""
    ...
    def parse_output(stdout: str) -> list[str]:
        """Parse jsminer output: try JSON array first, fallback to line-per-line."""
    ...
    async def execute(
        ...
        **kwargs,
        ...
        """Run JsMiner against all saved JS files for *target_id*.
        ...
        if await self.check_cooldown(target_id, container_name):
            log.info("Skipping jsminer — within cooldown period")
            return {
                "endpoints_found": 0,
                "assets_saved": 0,
                "params_saved": 0,
        ...
        # 3. Run JsMiner against each JS file
        for filepath in js_files:
            try:
                ...
                log.warning(f"jsminer failed on {filepath}: {exc}")
                continue
        ...
        log.info("jsminer complete", extra=stats)
        return stats
